...
{
	// some parameters
	std::string kernelSource = rascl::detail::kFocusPlanes::source;
	std::string kernelName = rascl::detail::kFocusPlanes::name;
	cl_uint initialValue = 7;
	// Triangle hostTriangle = make_triangle({0,0},{12.5,25},{25,0});
	Triangle hostTriangle = make_triangle({1,5},{10,1},{5,10});
	unsigned int npx = 25;
	...
	// create the host and device vectors.
	auto hostVec = std::vector<cl_float>(npx*npy, 0);
	...
	for (size_t i = 0; i < npy; ++i)
	{
		for (size_t j = 0; j < npx; ++j)
		{
			std::cout << (unsigned int)(hostVec[i*npx+j]*255) << " ";
		}
		std::cout << "\n";
	}
}